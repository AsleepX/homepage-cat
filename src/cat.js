import { clamp, collectPlatforms, firstLanding, findRoute, jumpHeight, contactAt, standingHeight } from './cat-world.js';
import { groundedPaw, walkingLeg } from './cat-pose.js';
import { overFace, fusionDwell } from './cat-fusion.js';

export function startCat(cat, track, options = {}) {
  const root = options.root || document.body;
  const events = new AbortController();
  const on = (target, name, listener, settings = {}) => target?.addEventListener(name, listener, {...settings, signal: events.signal});
  const collect = () => collectPlatforms(track, options);
  if (!cat || !track) return;
  ...
  const halfWidth = 28;
  const portrait = options.portrait || null;
  const portraitLabel = portrait?.getAttribute('aria-label');
  const dwell = fusionDwell();
  let fusionFrame = 0;
  let faceWaiting = false;
  let fused = false;
  let fusionAnimation = null;
  const earButtons = [...(options.earButtons || [])];
  ...
  function fuseWithPortrait() {
    ...
    portrait.setAttribute('aria-label', options.fusedLabel || 'Avatar with cat ears and whiskers');
  ...
  async function releaseFromPortrait(side, keyboard) {
    ...
    portrait.setAttribute('aria-label', portraitLabel);
    cancelJourney();
    const token = journey;
    busy = true;
    cat.hidden = false;
  ...
  earButtons.forEach(button => on(button, 'click', event => {
  ...
  on(window, 'pointerup', event => release(event));
  on(window, 'pointercancel', event => release(event, true));
  on(cat, 'keydown', event => { if (event.key === 'Escape' && roaming) { release(null, true); restoreHome(); } });
  ...
  contentChanges.observe(root, {
    subtree: true, childList: true, characterData: true, attributes: true, attributeFilter: ['style', 'class'],
  });
  on(document.fonts, 'loadingdone', terrainChanged);
  on(options.portraitImage, 'load', terrainChanged);
  root.querySelectorAll('img[data-cat-scene]').forEach(image => on(image, 'load', terrainChanged));
  on(window, 'resize', () => {
  ...
  on(window, 'blur', () => { if (drag) restoreHome(); });
  on(reducedMotion, 'change', () => { if (roaming) restoreHome(); else sleep(); });
  return {
    element: cat,
    reset() {
      fused = false;
      ...
      portrait?.classList.remove('is-cat');
      if (portrait) {
        if (portraitLabel === null) portrait.removeAttribute('aria-label');
        else portrait.setAttribute('aria-label', portraitLabel);
      }
      earButtons.forEach(button => { button.hidden = true; });
      cat.hidden = false;
      cat.classList.remove('is-fusing');
      restoreHome();
    },
    destroy() {
      this.reset();
