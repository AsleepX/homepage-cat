import { textContours, imageContour, contactAt, standingHeight } from './cat-surfaces.js';
export { contactAt, standingHeight } from './cat-surfaces.js';
import { photoPlatforms } from './cat-photo-world.js';
// The page is a set of one-way platforms: jumps pass through from below.
export const clamp = (value, min, max) => Math.max(min, Math.min(max, value));
export function collectPlatforms(track, options = {}) {
  ...
  root.querySelectorAll(options.platformSelector || '[data-cat-platform]').forEach((el, i) => {
    const r = rect(el);
    add(`rule-${i}`, r.left, r.right, r.top);
  });
  const portrait = imageContour(options.portraitImage);
  ...
  for (const [side,left,right] of [['left',.17,.43],['right',.63,.85]]) {
    const shoulder = imageContour(options.portraitImage, {left,right,top:.74,bottom:.95});
    if (shoulder) add(`portrait-${side}-shoulder`,shoulder.left,shoulder.right,shoulder.y,shoulder);
  }
  root.querySelectorAll('img[data-cat-scene]').forEach(image => {
  ...
  while ((text = walker.nextNode())) {
    if (!text.textContent.trim() || text.parentElement.closest('.cat, svg, script, style, noscript, [data-cat-ignore]' )) continue;
